...
import getCustomerPayments from "./get";
...
export default async function CustomerBillsPage(prop: Props) {
    ...
    const result = await getCustomerPayments(page, quantity, search)
    const {count: counts, data: payments, success, message} = result

    // Calculate stats
    const totalPayments = counts
                        ...
                        <CardContent className="p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-black text-slate-900 dark:text-white mt-1">{totalPayments}</p>
                                </div>
                        ...
                        <CardContent className="p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-black text-green-600 dark:text-green-400 mt-1">{verifiedPayments}</p>
                                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                                        {payments.length > 0 ? ((verifiedPayments / payments.length) * 100).toFixed(1) : 0}% dari halaman ini
                                    </p>
                                </div>
                        ...
                        <CardContent className="p-5">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-black text-yellow-600 dark:text-yellow-400 mt-1">{pendingPayments}</p>
                                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                                        {payments.length > 0 ? ((pendingPayments / payments.length) * 100).toFixed(1) : 0}% dari halaman ini
                                    </p>
                                </div>
                ...
                {!success && payments.length === 0 ? (
                    ...
                    <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                        <CardContent className="p-16 text-center">
                            ...
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Pembayaran Tidak Ditemukan</h2>
                            <p className="text-slate-500 dark:text-slate-400 mt-2">Tidak ada pembayaran untuk "{search}". Coba kata kunci lain.</p>
                        </CardContent>
                    </Card>
                ) : (
                                    ...
                                    <div className="flex items-start md:items-center gap-5 flex-[1.5]">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">
                                                Bill #{payment.bill_id} • {payment.bill ? `${getMonthName(payment.bill.month)} ${payment.bill.year}` : '-'}
                                            </p>
                                        </div>
                                    </div>
